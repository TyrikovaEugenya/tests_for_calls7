...
def test_user_flow_parametrized_firefox_webkit(page, get_film_url, device, throttling, geo, browser_type, request):
    report = {
        "test_name": "test_user_flow_firefox_webkit",
    ...
    with allure.step(f"Переходим на главную страницу {config.BASE_URL}"):
        try:
            ...
            pytest.fail(f"Не удалось загрузить {config.BASE_URL} за 30 сек", pytrace=False)
    ...
    with allure.step(f"Переходим на страницу фильма и собираем метрики для {get_film_url}"):
        try:
            ...
            pytest.fail(f"Видео не появилось на странице {get_film_url} за 15 сек", pytrace=False)
